...
function addRoom($roomName, $hasTV, $beds, $brojkvadrata){
	global $conn;
	$rarray = array();
	$stmt = $conn->prepare("INSERT INTO rooms (roomname, tv, beds, brojkvadrata) VALUES (?, ?, ?, ?)");
	$stmt->bind_param("ssss", $roomName, $hasTV, $beds, $brojkvadrata);
	if($stmt->execute()){
		return "true";
		// $rarray['sucess'] = "ok";
	}else{
